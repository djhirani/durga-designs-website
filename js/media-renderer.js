...
const DDRender = (() => {
  ...
  /* ── Constants (from config.js when available) ─────────────── */
  const WA_NUM     = (typeof DD_CONFIG !== 'undefined') ? DD_CONFIG.contact.whatsapp : '447907975847';
  const WA_BASE    = `https://wa.me/${WA_NUM}?text=`;
  const WA_GENERAL = (typeof DD_CONFIG !== 'undefined') ? DD_CONFIG.waGeneral : WA_BASE + encodeURIComponent('Hi Durga Designs, I would like to enquire.');
  ...
  /* ── Logo injection — premium lockup (mark + wordmark) ─────── */
  function injectLogo() {
    if (!DD_ASSETS.logo) return;
    const logo   = DD_ASSETS.logo.main;
    const logoLg = { src: 'images/optimized/logo.png', alt: logo.alt };

    document.querySelectorAll('.navbar-logo').forEach(el => {
      if (el.querySelector('.logo-lockup')) return;
      ...
      el.innerHTML =
        `<span class="logo-lockup">` +
          `<img class="logo-mark" src="${safeSrc(logo.src)}" alt="" aria-hidden="true" loading="eager" decoding="async" width="52" height="52" />` +
          `<span class="logo-text">` +
            `<span class="logo-main">${mainText}</span>` +
            (subText ? `<span class="logo-sub">${subText}</span>` : '') +
          `</span>` +
        `</span>`;
      el.setAttribute('aria-label', logo.alt || 'Durga Designs Home');
    });
  ...
  function renderCollectionsGrid(containerId) {
    ...
    const active = DD_ASSETS.products
      .filter(p => p.active)
      .sort((a, b) => {
        const catOrder = { abayas:1, 'ladies-suits':2, unstitched:3, 'laces-buttons':4, sarees:5, kids:6, modern:7, 'shoulder-bags':8, kurtas:9, 'wall-hangings':10 };
        const catA = catOrder[a.category] || 99;
        const catB = catOrder[b.category] || 99;
        return catA !== catB ? catA - catB : a.order - b.order;
      });
    ...
    // Apply ?cat= deep link now that the cards exist (main.js runs too
    // early — the grid is injected after it clicks the tab)
  ...
  function renderHeroImage(containerId) {
    ...
    el.innerHTML = `<img src="${safeSrc(hero.src)}" alt="${hero.alt}" loading="eager" />`;
  }
